...
{
    Task<LabourSummary?> ILabourLookup.FindAsync(int labourId, CancellationToken cancellationToken) =>
        Task.FromResult<LabourSummary?>(new LabourSummary(labourId, 1, "Sample Labour", "InductionPending", 1));

    Task<ContractorSummary?> IContractorLookup.FindAsync(int contractorId, CancellationToken cancellationToken) =>
        Task.FromResult<ContractorSummary?>(new ContractorSummary(contractorId, "Sample Contractor", "9999999999"));

    Task<ProjectSummary?> IProjectDirectory.FindAsync(int projectId, CancellationToken cancellationToken) =>
        Task.FromResult<ProjectSummary?>(new ProjectSummary(projectId, "PRJ-001", "Sample Project"));

    Task<WorkCategorySummary?> IWorkCategoryLookup.FindAsync(int workCategoryId, CancellationToken cancellationToken) =>
